...
const CACHE_TTL = 60 * 60 * 1000; // 1 hour in-memory TTL

// ── Load cache from DB on startup ────────────────────────────────────────────
async function loadCacheFromDB() {
  try {
    const r = await pool.query('SELECT data, device_names, built_at FROM ticketsmodule_equipment_cache WHERE id=1');
    if (r.rows.length) {
      cache = r.rows[0].data;
      deviceNamesCache = r.rows[0].device_names || {};
      cacheTs = new Date(r.rows[0].built_at).getTime();
      console.log(`✅ Equipment cache loaded from DB: ${cache.length} items (built ${new Date(cacheTs).toLocaleString('ru')})`);
      return true;
    }
  } catch(e) {
    console.error('loadCacheFromDB error:', e.message);
  }
  return false;
}

// ── Save cache to DB ──────────────────────────────────────────────────────────
async function saveCacheToDB(items, devNames) {
  try {
    await pool.query(
      `INSERT INTO ticketsmodule_equipment_cache (id, data, device_names, built_at)
       VALUES (1, $1, $2, NOW())
       ON CONFLICT (id) DO UPDATE SET data=$1, device_names=$2, built_at=NOW()`,
      [JSON.stringify(items), JSON.stringify(devNames)]
    );
    console.log('✅ Equipment cache saved to DB');
  } catch(e) {
    console.error('saveCacheToDB error:', e.message);
  }
}
...
async function buildCache(force = false) {
  ...
  // Try DB cache first (fast, survives redeploys)
  if (!cache && !force) {
    const loaded = await loadCacheFromDB();
    if (loaded) return;
  }
  ...
  try {
    ...
    // Save to DB for fast startup next time
    await saveCacheToDB(cache, deviceNamesCache);
  } catch(e) {
    loadError = e.message;
...
// POST /equipment/refresh — force full rebuild + save to DB
router.post('/refresh', requireAuth(['admin','coordinator']), async (req, res) => {
  try {
    await pool.query('DELETE FROM ticketsmodule_equipment_cache WHERE id=1');
    await pool.query('DELETE FROM ticketsmodule_equipment_geo WHERE geocode_failed=true');
  } catch(e) {}
  cache = null; cacheTs = 0;
  res.json({ ok: true, message: 'Кеш сброшен, данные будут перезагружены' });
});
