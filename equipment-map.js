...
// Финальные (неактивные) стадии — как в модуле «Сервисные заявки»: только
// Успех/Отказ. Стадию DT1058_11:4 (Претензия) модуль считает АКТИВНОЙ, поэтому
// на карте её тоже не исключаем — иначе счётчики расходятся.
const isFinalStage = s => /:(SUCCESS|FAIL)$/i.test(String(s || ''));
